...
DecayConstant = 0.001; % Decay constant for the impulses in vn
 %round(3 * 10e-2 * Fs); % 30ms, dangelo example last graph
...
%                        p.SustainThreshold = 0.86; %0.9 [0 1] gain has to be higher than so sustain acts
                ...
                p.inSnippetBuffer = zeros(p.WindowsLength, 1); % Input buffer to hold a snippet
                p.CopyOf_inSnippetBuffer = zeros(p.WindowsLength, 1);
                p.outSnippet = zeros(p.WindowsLength, 1); % Not really used, just to hold values for plotting
...
%   - Posible future tturnable parameters:
            %p.WindowsLength : Must change the size of many buffers if you
            %reassigne it! A LOT OF LOAD CPU
