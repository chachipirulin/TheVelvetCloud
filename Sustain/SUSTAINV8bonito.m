% Comments at bottom
    ...
    classdef SUSTAINV8bonito < audioPlugin         
...
%__________________ Low Pass coefs for convolving snippet _________________%
    % fcut
Fcut= 5000;
Wn =  5000 / 44100;   
order = 3;
...
function out = process(p,in)      
            ...
            out_mono = zeros(size(in_mono));         
                ...
                % If buffer is NOT full (ie: initialization or gain > threshold)
               ...
               p.SamplesAlreadyInBuffer = p.SamplesAlreadyInBuffer + length(in_mono);  
                ...
                if p.SamplesAlreadyInBuffer > p.BufferLength
                    ...
                    p.inSnippetBuffer = p.inSnippetBuffer .* p.WindowsStatic;
                     ...
                     p.CopyOf_inSnippetBuffer = p.inSnippetBuffer .* p.playbackVoiceGain; % InSnippetBuff will be as high as playbackVoiceGain max
        ...
        out_mono(n,1) =  p.playbackVoice(1, 1);
        ...
        %% Check conditions for getting a new convolving snippet
        
    if any((in_mono) > p.SustainThreshold ) && p.ReadyToTakeSnippet == true && median(abs(in_mono))>0.21 && mean(abs(in_mono))>0.30
        
                % If a value of our snippet was higher than thres, then add 1 to  first
        % rise
        p.FirstRise = p.FirstRise + 1;
        p.ReadyToTakeSnippet = false;
         p.IsSnippetBufferFull = false; % RETHINK THIS PHILOSPHY FOR BETTER NAMING BETTTER READY TO TAKE SNIPPET
...
%             end
        ...
        %If there is no one higher than threshold
    elseif any((in_mono) < p.SustainThreshold) == true 
...
% audioTestBench SUSTAINV8bonito.m
